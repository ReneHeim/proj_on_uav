...
Usage:
    python -m scripts.archive.legacy_week1_gpu.run_week1_gpu_sift \\
        --set 0000SET \\
...
DEFAULT_METASHAPE_CORRECTION = {name: 1.0 for name in METASHAPE_MS_BAND_NAMES}
...
def load_metashape_correction(path: Path | None, radiometry_mode: str) -> dict[str, float]:
    if radiometry_mode == "micasense_panel":
...
def _warp_work_share_seed(
    ...
    # MicaSense RedEdge-P raw/imager order is Blue, Green, Red, NIR, Red edge.
    # Agisoft/Pix4D processed GeoTIFFs are wavelength order:
    # Blue, Green, Red, Red edge, NIR. Write that order for comparability.
    multispec = np.stack([aligned[..., index] for index in (0, 1, 2, 4, 3)], axis=-1)
    factors = np.array(
        [float(correction.get(name, 1.0)) for name in ("Blue", "Green", "Red", "Red edge", "NIR")],
        dtype=np.float32,
    )
    multispec = multispec * factors[None, None, :]
    ...
    with rasterio.open(out_path, "w", **profile) as dst:
        ...
        dst.update_tags(
            ...
            panel_capture_id=cap.uuid,
            raw_band_order="Blue, Green, Red, NIR, Red edge, Panchro",
            output_band_order="Blue, Green, Red, Red edge, NIR",
...
def process_capture(
    ...
    args = (
        ...
        radiometry_mode,
        correction,
        str(correction_source) if correction_source else None,
    )
    ...
    out_path = _warp_work_share_seed(*args)
    return {
        "seed": str(seed),
        "output": out_path,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "micasense_panel writes panel-calibrated MicaSense reflectance. "
    ...
    log.info("[RADIOMETRY] mode=%s correction=%s", args.radiometry_mode, correction)
