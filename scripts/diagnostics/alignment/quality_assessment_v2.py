...
def run_worker(capture_seed: Path, ref_band_idx: int = 5) -> dict:
    ...
    band_paths = [
        str(capture_seed.with_name(capture_seed.name.replace("_1.tif", f"_{s}.tif")))
    ...
    log.info(
        "[WORKER] sift=%.2fs match=%.2fs total=%.2fs n_inliers_total=%d",
        gpu_data["t_sift_s"],
        gpu_data["t_match_s"],
        gpu_data["t_total_s"],
...
def apply_warps_multispec(capture_seed: Path, warps: list, ref_band_idx: int = 0) -> np.ndarray:
    ...
    for i, b in enumerate(bands):
        ...
        log.info(
            "[WARP] band %d: shape=%s -> (%d, %d)  mean before=%.3f after=%.3f",
            i,
            b.shape,
            target_h,
            target_w,
    ...
    log.info("[WARP] valid all-band overlap: %.2f%%", 100.0 * float(valid_overlap.mean()))
...
def main() -> int:
    ...
    for m in gpu_data["matches_per_band"]:
        log.info(
            "[GPU:band %d] n_features=%d n_match=%d n_inliers=%d scale=%.5f "
    ...
    log.info("=" * 70)
    log.info("RESULT: similarity returncode = %d (0=pass, 1=fail, 2=shape mismatch)", sim_rc)
    return sim_rc
