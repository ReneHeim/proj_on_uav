...
from skimage.feature import SIFT, match_descriptors as _sk_match
...
def _make_kornia_sift(num_features: int = 5000, device: str = "cuda"):
    ...
    def kornia_sift(self, ref=5, min_matches=10, verbose=0,
                    err_red=10.0, err_blue=12.0, err_LWIR=12.0):
        import torch
        from kornia.feature import match_smnn
        from kornia.feature.laf import get_laf_center
        from skimage.transform import ProjectiveTransform
        t_total = time.perf_counter()
        log.info("[GPU-SIFT] enter  ref=%d  num_features=%d  device=%s",
                 ref, num_features, device)
        ...
        log.info("[GPU-SIFT] bands tensor: shape=%s  dtype=%s  device=%s",
                 bands_t.shape, bands_t.dtype, bands_t.device)
        ...
        for ix in img_index:
            ...
            log.info("[GPU-SIFT] band %d (%s): smnn matches=%d  (%.2fs)",
                     ix, self.images[ix].band_name, n_match, time.perf_counter() - t_b)
            if n_match == 0:
            ...
            try:
                filtered_kpi, filtered_kpr, filtered_match, _ = self.filter_keypoints(
                    list(kp_ix), list(kp_ref_full), match_pairs,
                    warp_matrices_calibrated[ix], scale, scale_i,
                    threshold=err_red if ix <= 5 else err_blue)
            except Exception as exc:
                log.warning("[GPU-SIFT] band %d filter_keypoints failed: %s", ix, exc)
                out_warps[ix] = warp_matrices_calibrated[ix]
                continue
            log.info("[GPU-SIFT] band %d filtered=%d (min=%d)", ix, len(filtered_match), min_matches)

            if len(filtered_match) >= min_matches:
                kpi_arr = np.asarray(filtered_kpi)
                kpr_arr = np.asarray(filtered_kpr)
                kpi_use, kpr_use, _, _ = self.find_inliers(kpi_arr, kpr_arr, np.asarray(filtered_match))
                P = estimate_transform('projective',
                                       (scale * kpr_use)[:, ::-1],
                                       (scale_i * kpi_use)[:, ::-1])
            else:
...
def _make_patched_sift(max_kp: int | None):
    ...
    def patched(self, ref=5, min_matches=10, verbose=0,
                err_red=10.0, err_blue=12.0, err_LWIR=12.0):
        t_total = time.perf_counter()
        ...
        if max_kp is not None:
            ...
            n_ref = len(descriptor_extractor.keypoints) if descriptor_extractor.keypoints is not None else 0
        ...
        for ix in img_index:
            ...
            n_kp = _limit_keypoints(descriptor_extractor, max_kp) if max_kp is not None else len(descriptor_extractor.keypoints)
            keypoints.append(descriptor_extractor.keypoints)
            descriptors.append(descriptor_extractor.descriptors)
            log.info("[SIFT] band %d (%s): total %.2fs  keypoints=%d",
                     ix, self.images[ix].band_name, time.perf_counter() - t_band, n_kp)
        ...
        for m, k, ix, t_thresh in zip(matches, keypoints, img_index, filter_tr):
            ...
            filtered_kpi, filtered_kpr, filtered_match, err = self.filter_keypoints(
                k, keypoints_ref, m, warp_matrices_calibrated[ix], scale, scale_i, threshold=t_thresh)
            log.info("[SIFT] band %d filter_keypoints: %.2fs  filtered=%d (min=%d)",
                     ix, time.perf_counter() - t_b, len(filtered_match), min_matches)
            if len(filtered_match) > min_matches:
                t_b = time.perf_counter()
...
def _run_with_timeout(fn, timeout_s: int, label: str) -> dict:
    ...
    try:
        ...
        return {"status": "error", "elapsed_s": time.perf_counter() - t0,
                "error": repr(exc), "traceback": traceback.format_exc()}


def benchmark_capture(capture_seed: Path, panel_seed: Path | None = None,
    ...
    if use_subprocess:
        return _benchmark_capture_subprocess(
            capture_seed, methods, per_method_timeout_s, results
        )
    ...
    for name, max_kp in methods:
        ...
        if name == "kornia_sift_5000":
            ...
            try:
                band_paths = [
                    str(_mpp.band_path_from_blue(capture_seed, s))
                    for s in _mpp.BAND_SUFFIXES
                ]
                ...
                results["methods"][name] = {
                    "status": "timeout", "elapsed_s": elapsed, "max_kp": max_kp,
                }
            except Exception as exc:
                elapsed = time.perf_counter() - t0
                log.exception("[GPU] ERROR after %.1fs", elapsed)
                results["methods"][name] = {
                    "status": "error", "elapsed_s": elapsed, "max_kp": max_kp,
                    "error": repr(exc),
                }
            continue
        # CPU methods
...
def _run_one_method_subprocess(capture_seed: Path, max_kp: int | None, queue,
    ...
    try:
        ...
        warps = cap.SIFT_align_capture(ref=5, min_matches=8, verbose=0)
        elapsed = time.perf_counter() - t0
        queue.put({"status": "ok", "elapsed_s": elapsed, "max_kp": max_kp,
                   "warps": len(warps)})
    except Exception as exc:
...
def _benchmark_capture_subprocess(capture_seed, methods, timeout_s, results) -> dict:
    ...
    for name, max_kp in methods:
        log.info("-" * 60)
        log.info("METHOD (subprocess): %s  max_kp=%s  timeout=%ds",
                 name, max_kp, timeout_s)
        log.info("-" * 60)
        q = _mp.Queue()
        p = _mp.Process(
        ...
        if p.is_alive():
            ...
            try:
                ...
                payload = {"status": "error", "error": "no result from subprocess",
                           "exitcode": p.exitcode}
            payload["max_kp"] = max_kp
            payload["elapsed_s"] = elapsed
            log.info("[RESULT] %s: %s  elapsed=%.1fs", name, payload.get("status"), elapsed)
            if "gpu_sift_s" in payload:
                log.info("[RESULT]   GPU detail: sift=%.2fs  match=%.2fs  total=%.2fs  matches=%s",
                         payload["gpu_sift_s"], payload["gpu_match_s"],
                         payload["gpu_total_s"], payload.get("matches_per_band"))
...
def write_markdown(all_results: list[dict]) -> None:
    ...
    for r in all_results:
        ...
        for name, _ in METHODS:
            ...
            speedup = f"{baseline_s / elapsed:.1f}x" if baseline_s and elapsed and status == "ok" else "-"
...
def main():
    ...
    if not args.skip_fast:
        all_results.append(benchmark_capture(
            args.fast_capture,
