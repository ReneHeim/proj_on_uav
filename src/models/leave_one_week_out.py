...
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s",
                    handlers=[logging.FileHandler(LOGS_DIR / f"leave_one_week_out_{ts}.log"),
...
def load_features():
    ...
    for name in ["M1_nadir_bands", "M3_multiangular_vza", "M5_angular_contrast"]:
        ...
        data[name] = df
        logging.info(f"  {name}: {df.height} rows, {len([c for c in df.columns if c not in EXCLUDE])} features")
    return data
...
def run_lowo(data, year):
    ...
    for name, df in data.items():
        ...
        for test_week in weeks:
            ...
            row = {
                "feature_set": name, "year": year,
                "test_week": test_week,
                "n_train": len(y_train), "n_test": len(y_test),
                "pos_train": y_train.mean(), "pos_test": y_test.mean(),
                "AUROC": auroc_test, "AUROC_train": auroc_train,
        ...
        if name_results:
            ...
            # Δ vs M1
            logging.info(
                f"  {name} {year} LOWO mean: AUROC={mean_auroc:.3f} ± {mean_std:.3f}"
            )
...
def main():
    ...
    logging.info(f"{'='*70}")
    logging.info(f"  {'Set':>4s} {'Year':>4s}  {'Test AUROC':>10s}  {'Train':>7s}  {'Gap':>6s}  {'F1':>7s}")
    ...
    for yr in [2024, 2025]:
        for fs in ["M1_nadir_bands", "M3_multiangular_vza", "M5_angular_contrast"]:
            ...
            logging.info(f"  {fs[:5]:>4s} {yr:>4s}  {m_test:>9.3f} ± {subset['AUROC'].std():.4f}  {m_train:>6.3f}  {m_gap:>5.3f}  {m_f1:>6.3f}")
    ...
    for yr in [2024, 2025]:
        ...
        if m3.height > 0 and m1.height > 0:
            ...
            logging.info(f"  {yr}: M3={m3['AUROC'].mean():.3f}, M1={m1['AUROC'].mean():.3f}, Δ=+{d:.3f}")
