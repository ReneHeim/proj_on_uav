    ...
    def validate_config(cls, config: AttrDict) -> List[str]:
        ...
        for field_path, expected_type in cls.REQUIRED_FIELDS.items():
            ...
            if value is None:
                ...
                errors.append(f"Field {field_path} must be {expected_type.__name__}, got {type(value).__name__}")
...
def config_object(config_path: Union[str, Path]) -> AttrDict:
    ...
    for path_key in ['cam_path', 'dem_path', 'orthophoto_path', 'mosaic_path', 
                     'ground_truth_coordinates', 'polygon_file_path']:
