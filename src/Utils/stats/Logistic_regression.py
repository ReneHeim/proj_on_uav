...
def preprocess_healthy_diseased(
    df_h,
    df_d,
    sample_size=500_000,
    random_state=42,
...
def _auroc_fast(
    ...
    def _geometry_match_indices(df_non_nadir: pd.DataFrame, y_non_nadir: np.ndarray) -> np.ndarray:
        ...
        kb_v = KBinsDiscretizer(
            n_bins=v_bins,
            encode="ordinal",
            strategy=match_strategy,
            quantile_method="averaged_inverted_cdf",
        )
        kb_r = KBinsDiscretizer(
            n_bins=r_bins,
            encode="ordinal",
            strategy=match_strategy,
            quantile_method="averaged_inverted_cdf",
    ...
    if geometry_match:
        ...
        logging.info(
            f"geometry_match=True → kept {X_non_nadir.shape[0]} non-nadir rows after VZA×RAA balancing."
        )
    ...
    if same_size:
        ...
        logging.info(
            f"same_size=True → non-nadir size set to N={N} "
    ...
    pipe_nadir = Pipeline(
        steps=[
            ("prep", ColumnTransformer(transformers=[scale_bands], sparse_threshold=1.0)),
            (
                "clf",
                LogisticRegression(
    ...
    prep_main = ColumnTransformer(
        transformers=[scale_bands, bin_vza_feat, bin_raa_feat], sparse_threshold=1.0
    )
    ...
    if group_col is None:
        ...
        inner_cv = StratifiedKFold(
            n_splits=max(3, min(5, n_splits - 1)), shuffle=True, random_state=random_state + 1
        )
    else:
    ...
    def _cv_auc(pipe, X_, y_, label, groups=None):
        ...
        if group_col is None:
            scores = cross_val_score(
                pipe, X_, y_, cv=outer_cv, scoring=auc_scorer, n_jobs=-1, pre_dispatch="2*n_jobs"
            )
        else:
            scores = cross_val_score(
                pipe,
...
def _calculate_cohens_d(d):
    ...
    top_str_keys = {f"vza_bin={vb}_raa_bin={rb}": float(val) for (vb, rb), val in top.items()}
...
def format_logistic_results(
    lr_out: Union[dict, pl.DataFrame], *, shape: str = "wide", top_k: int = 5
) -> pl.DataFrame:
    """
    ...
    if shape == "wide":
        ...
        for i in range(1, top_k + 1):
            preferred_order += [f"top{i}_vza_bin", f"top{i}_raa_bin", f"top{i}_d", f"top{i}_label"]
