...
for path in tqdm(paths):
    ...
    for i, target in tqdm({'lai':'lai_cat_a', 'chl':'chl_cat_a', 'nbi':'nbi_cat_a', 'glai':'glai_cat_a'}.items()):
        ...
        for j, model in tqdm(models.items()):
            ...
            cv_score_accuracy = cross_val_score(model, X = X, y = Y[target], cv = cv_class[res=='20cm'], n_jobs= -1, scoring = 'accuracy')
            cv_score_f1 = cross_val_score(model, X = X, y = Y[target], cv = cv_class[res=='20cm'], n_jobs= -1, scoring = 'f1_weighted')
    ...
    data_base = np.transpose(np.array([dates, resolution, parameter, model_list, approaches, dataset_list, vza_angle_list, vaa_angle_list, mean_accuracy, sd_accuracy, mean_f1, sd_f1]))
    data_base = pd.DataFrame(data_base, columns = ['dates', 'resolution', 'parameter', 'model', 'approaches', 'dataset_list', 'vza_angle_list', 'vaa_angle_list', 'mean_accuracy', 'sd_accuracy', 'mean_f1', 'sd_f1'])
    data_base.to_csv(os.path.join(out_path, 'data_summary_table_with_soil_irregular_bin.csv'))