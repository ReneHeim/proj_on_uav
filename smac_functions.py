...
def getdualex(filepath, seperator):
    ...
    df.columns = ['date', 'time', 'lon', 'lat', 'alt', 'sat_qual', 'temp', 'group', 'measure', 'side', 'chl', 'flav',
                  'anth', 'nbi', 'calib']
...
def calculate_indices(database_path, method='classification', date='20200907'):
    ...
    # Rename columns
    data[["band1", "band2", "band3", "band4", "band5", "band6", "band7", "band8", "band9", "band10"]] = data[["band1",
                                                                                                              "band2",
                                                                                                              "band3",
                                                                                                              "band4",
                                                                                                              "band5",
    ...
    mtvi = (1.2 * (nir842 - green560) - 2.5 * (
                red668 - green560))  # https://www.l3harrisgeospatial.com/docs/narrowbandgreenness.html
    mtvi2 = 1.5 * mtvi / np.sqrt((2 * nir842 + 1) ** 2 - (6 * nir842 - 5 * np.sqrt(red668) - 0.5))

    msavi2 = 0.5 * (2 * nir842 + 1 - np.sqrt((2 * nir842 + 1) ** 2 - 8 * (
                nir842 - red668)))  # https://www.isprs.org/proceedings/xxxv/congress/comm7/papers/21.pdf
    ...
    X.columns = ['blue444', 'blue475', 'green531', 'green560', 'red650', 'red668', 're705', 're717', 're740', 'nir842',
                 'ci', 'evi', 'gndvi', 'mcari', 'mdnblue', 'mtci', 'ndre740', 'ndre842', 'ndvi', 'pri', 'rep',
                 're750_700',
                 'tcari', 'osavi', 'tcari_osavi', 'sccci', 're_ndvi', 'msr', 're_msr', 'ci_green', 'ci_re', 'mtvi',
                 'mtvi2',
                 'msavi2', 'ndvi_re', 'cccci']
