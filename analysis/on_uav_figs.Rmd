...
class_df$vza_group[class_df$vza_group == 'all'] <- 'all_vza'

class_df$vaa_group[class_df$vaa_group == 'mosaic'] <- 'orthomosaic'
class_df$vaa_group[class_df$vaa_group == 'all'] <- 'all_vaa'
...
## 3.3 Which dataset performs best?
...
## 3.4 Do the leaf parameter accuracies differ between datasets?
...
# 3 Figure 3 - VZA and VAA Distribution
...
#feather_path <- "D:\\OneDrive - Institut für Zuckerrübenforschung\\projects\\proj_on_uav\\data\\proc\\"

feather_path <- "C:\\Users\\heim\\OneDrive - Institut für Zuckerrübenforschung\\projects\\proj_on_uav\\data\\proc\\"
...
df_nadir$vaa_grp[df_nadir$vaa_grp == '[240, 300)'] <- 'ss'
df_nadir$vaa_grp[df_nadir$vaa_grp == '[60, 120)'] <- 'ss'

df_nadir$vaa_grp[df_nadir$vaa_grp == '[120, 240)'] <- 'fs'

df_nadir$vaa_grp[df_nadir$vaa_grp == '[300, 360]'] <- 'bs'
df_nadir$vaa_grp[df_nadir$vaa_grp == '[0, 60)'] <- 'bs'
...
df_all$vaa_grp[df_all$vaa_grp == '[240, 300)'] <- 'ss'
df_all$vaa_grp[df_all$vaa_grp == '[60, 120)'] <- 'ss'

df_all$vaa_grp[df_all$vaa_grp == '[120, 240)'] <- 'fs'

df_all$vaa_grp[df_all$vaa_grp == '[300, 360]'] <- 'bs'
df_all$vaa_grp[df_all$vaa_grp == '[0, 60)'] <- 'bs'
...
ggplot(aes(x=vaa_deg, fill=date)) + 
  ...
  theme(legend.position = "none",legend.title=element_blank()) -> p3
...
ggplot(aes(x=vaa_deg, fill=date)) + 
  geom_histogram(alpha=.5) +
  xlab("VAA [°]") + ylab("")+
  facet_wrap(vza_grp ~. ) -> p4

p1 + p2 + p3 + p4 + plot_layout(ncol = 2, guides = "collect")

out <- "C:\\Users\\heim\\OneDrive - Institut für Zuckerrübenforschung\\projects\\proj_on_uav\\docs\\manuscript\\final\\figures\\fig3_vza_vaa_distribution\\"

ggsave(paste0(out, "fig3_vza_vaa.png"), width = 30, height = 20, unit = "cm", dpi = 300)
...
## 4.1 Reshaping and cleaning reference data for boxplots
...
```{r}
df_1m_lai_et_nadir$vaa_group2 = factor(df_1m_lai_et_nadir$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))
df_1m_lai_et_noo$vaa_group2 = factor(df_1m_lai_et_noo$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))

plai_nadir <- ggplot(df_1m_lai_et_nadir, aes(vza_group, mean_f1, label=mean_f1)) + 
  geom_point(aes(shape = dates, colour = dates), alpha = 0.5, size = 2.5)+
  scale_y_continuous(breaks=seq(from = 0.6, to = 1, by = 0.1), limits=c(0.6,1))+
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="LAI | ET | NADIR")
...
plai_all <- ggplot(df_1m_lai_et_noo, aes(vza_group, mean_f1, label=mean_f1)) + 
  ...
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="LAI | ET | NOO")

plai_nadir + plai_all + plot_layout(ncol = 1, guides = "collect")

out <- "C:\\Users\\heim\\OneDrive - Institut für Zuckerrübenforschung\\projects\\proj_on_uav\\docs\\manuscript\\final\\figures\\fig5_lai_glai_classification\\"

ggsave(paste0(out, "fig5_lai_classif.pdf"), width = 30, height = 20, unit = "cm", dpi = 300)
```
...
```{r}
df_1m_glai_et_nadir$vaa_group2 = factor(df_1m_glai_et_nadir$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))
df_1m_glai_et_noo$vaa_group2 = factor(df_1m_glai_et_noo$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))
...
pglai_all <- ggplot(df_1m_glai_et_noo, aes(vza_group, mean_f1, label=mean_f1)) + 
  ...
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="GLAI | ET | NOO")

pglai_nadir + pglai_all + plot_layout(ncol = 1, guides = "collect")

ggsave(paste0(out, "fig5_glai_classif.pdf"), width = 30, height = 20, unit = "cm", dpi = 300)
```
...
```{r}
df_1m_lcc_et_nadir$vaa_group2 = factor(df_1m_lcc_et_nadir$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))
df_1m_lcc_et_noo$vaa_group2 = factor(df_1m_lcc_et_noo$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))

plcc_nadir <-ggplot(df_1m_lcc_et_nadir, aes(vza_group, mean_f1, label=mean_f1)) + 
  geom_point(aes(shape = dates, colour = dates), alpha = 0.5, size = 2.5)+
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="LCC | ET | NADIR")

plcc_all <-ggplot(df_1m_lcc_et_noo, aes(vza_group, mean_f1, label=mean_f1)) + 
  geom_point(aes(shape = dates, colour = dates), alpha = 0.5, size = 2.5)+
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="LCC | ET | NOO")

plcc_nadir + plcc_all + plot_layout(ncol = 1, guides = "collect")
...
ggsave(paste0(out, "fig6_lcc_classif.pdf"), width = 30, height = 20, unit = "cm", dpi = 300)
```
...
```{r}
df_1m_nbi_et_nadir$vaa_group2 = factor(df_1m_nbi_et_nadir$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))
df_1m_nbi_et_noo$vaa_group2 = factor(df_1m_nbi_et_noo$vaa_group, levels=c('orthomosaic','all_vaa','fs','bs', 'ss'))

pnbi_nadir <- ggplot(df_1m_nbi_et_nadir, aes(vza_group, mean_f1, label=mean_f1)) + 
  geom_point(aes(shape = dates, colour = dates), alpha = 0.5, size = 2.5)+
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="NBI | ET | NADIR")

pnbi_all <- ggplot(df_1m_nbi_et_noo, aes(vza_group, mean_f1, label=mean_f1)) + 
  geom_point(aes(shape = dates, colour = dates), alpha = 0.5, size = 2.5)+
  geom_text(hjust=-0.4, vjust=0.5, size=3.5, check_overlap = TRUE)+
  facet_grid(. ~ vaa_group2 , scales = "free", space = "free")+
  labs(title="NBI | ET | NOO")

pnbi_nadir + pnbi_all + plot_layout(ncol = 1, guides = "collect")

ggsave(paste0(out, "fig6_nbi_classif.pdf"), width = 30, height = 20, unit = "cm", dpi = 300)
```

## 5.5 Listing f1-scores
...
fi_df$vza_group[fi_df$vza_group == 'all'] <- 'all_vza'

fi_df$vaa_group[fi_df$vaa_group == 'mosaic'] <- 'orthomosaic'
fi_df$vaa_group[fi_df$vaa_group == 'all'] <- 'all_vaa'
...
ggplot(df_fi_noo, aes(x = bands, y = rel_importance, fill = parameter, width = 0.5)) +
  ...
  scale_fill_manual(values=c("#22b036", "#94B76B", "#c031ad", "#c682bd"))+
  theme_bw()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(y="Rel. Imp.", x="", title = "NOO | ET | 20200906")+
  facet_wrap(vza_group ~ vaa_group)+
  theme(legend.title=element_blank())

out <- "C:\\Users\\heim\\OneDrive - Institut für Zuckerrübenforschung\\projects\\proj_on_uav\\docs\\manuscript\\final\\figures\\fig7_rel_feature_n_day1\\"

ggsave(paste0(out, "s16_feature_noo_et_d1.png"), width = 30, height = 20, unit = "cm", dpi = 300)
...
ggplot(df_fi_n, aes(x = bands, y = rel_importance, fill = parameter, width = 0.5)) +
  ...
  theme_bw()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(y="Rel. Imp.", x="", title = "N | ET | 20200906")+
  facet_wrap(vza_group ~ vaa_group)+
  theme(legend.title=element_blank())

ggsave(paste0(out, "fig7_feature_n_et_d1.png"), width = 30, height = 20, unit = "cm", dpi = 300)
...
ggplot(df_fi_noo_2, aes(x = bands, y = rel_importance, fill = parameter, width = 0.5)) +
  ...
  theme_bw()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(y="Rel. Imp.", x="", title = "NOO | ET | 20200907")+
  facet_wrap(vza_group ~ vaa_group)+
  theme(legend.title=element_blank())
...
ggsave(paste0(out, "s17_feature_noo_et_d2.png"), width = 30, height = 20, unit = "cm", dpi = 300)
...
ggplot(df_fi_n_2, aes(x = bands, y = rel_importance, fill = parameter, width = 0.5)) +
  ...
  theme_bw()+
  theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(y="Rel. Imp.", x="", title = "N | ET | 20200907")+
  facet_wrap(vza_group ~ vaa_group)+
  theme(legend.title=element_blank())

ggsave(paste0(out, "fig8_feature_n_et_d2.png"), width = 30, height = 20, unit = "cm", dpi = 300)
```
...
ggplot( aes(wavelength, reflectance, colour = vza_grp)) +
  geom_line(aes(colour=vza_grp), size = 0.1)+
  geom_point(size = 0.5)+
...
ggsave(filename = paste0(out, "spectra_nadir_vza.png"), width = 
         30, height = 20, dpi = 300, units = "cm", device='png')
...
ggplot( aes(wavelength, reflectance, colour = vaa_grp)) +
  geom_line(aes(colour=vaa_grp), size = 0.1)+
  geom_point(size = 0.5)+
  scale_x_continuous(breaks = seq(400, 900, 100))+
  facet_wrap(plot ~ date)+
